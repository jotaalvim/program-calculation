...
\begin{eqnarray*}
\xymatrix{
  \ensuremath{\Conid{Exp}\;\Conid{S}\;\Conid{S}}\ar[d]_{Post}\ar[l]^{out} & S + S \times (\ensuremath{\Conid{Exp}\;\Conid{S}\;\Conid{S}})^*\ar[ll]_{\ensuremath{\mathsf{in}_{\tiny\ \textit{Exp}}}}\ar[d]^{id + id \times(Post^*)}\\
  (S^*)^*   & S + S \times ((S^*)^*)^*\ar[d]^{id + id \times(concat^*)} \\
            & S + S \times (S^*)^*\ar[d]^{id + mete} \\
            & S + (S^*)^*\ar[ul]^{(either \ensuremath{\Varid{singl}\comp \Varid{singl}} id )}
}
\end{eqnarray*}
...
\end{hscode}\resethooks
\end{document}
Numa primeira tentativa fizemos uma função present da seguinte forma, para além de a função nao cumpr